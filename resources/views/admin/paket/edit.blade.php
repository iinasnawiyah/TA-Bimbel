...
<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
	<div class="clearfix">
		<div class="pull-left">
			<h4 class="text-blue">Form Edit Paket</h4>
			<p class="mb-30 font-14"></p>
		</div>
	</div>
	<!-- <form method="POST" action="{{ route('paket.update',$data->id_paket) }}"> -->
	<form method="POST" action="{{ route('kelpaket.update',$data->id_paket) }}">
		...
		<div class="form-group row">
			...
			<div class="col-sm-12 col-md-1">
				<input <?php for ($i = 0; $i < count($kat_kelas); $i++) {
							if ($kat_kelas[$i] == $k->id_kelas) {
								echo "checked";
							}
						} ?> class="form-control" type="checkbox" name="id_kelas[]" value="{{$k->id_kelas}}">
			</div>
			@endforeach
		</div>
		<div class="form-group row">
